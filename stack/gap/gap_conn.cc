...
#define GAP_CCB_FLAGS_CONN_DONE 0x0E
  ...
  tL2CAP_COC_CFG_INFO local_coc_cfg; /* local configuration for LE Coc */
  tL2CAP_COC_CFG_INFO peer_coc_cfg;  /* local configuration for LE Coc */
} tGAP_CCB;
...
void gap_tx_complete_ind(uint16_t l2cap_cid, uint16_t sdu_sent) {
  ...
  else if ((p_ccb->con_state == GAP_CCB_STATE_CONNECTED) && (sdu_sent >= 1))
  {
    ...
    p_ccb->p_callback (p_ccb->gap_handle, GAP_EVT_TX_DONE, nullptr);
  }
}
